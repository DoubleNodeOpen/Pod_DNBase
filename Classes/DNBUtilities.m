...
#import "NSDate+DNBUtils.h"

@interface DNBUtilities()
{
    DNBLogLevel             logDebugLevel;
...
@implementation DNBUtilities

+ (id<DNBApplicationProtocol>)appDelegate
{
    return (id<DNBApplicationProtocol>)[[UIApplication sharedApplication] delegate];
}

+ (DNBUtilities*)sharedInstance
{
    static dispatch_once_t  once;
    static DNBUtilities*    instance = nil;
    
    dispatch_once(&once, ^{
        instance = [[DNBUtilities alloc] init];
    });
...
+ (NSString*)appendNibSuffix:(NSString*)nibNameOrNil withDefaultNib:(NSString*)defaultNib
{
    ...
    return [self appendNibSuffix:nibNameOrNil];
}
...
+ (void)runAfterDelayBlock:(NSArray*)arrayBlock
{
    ...
    [DNBUtilities runBlock:arrayBlock[1]];
}
...
+ (void)runOnMainThreadAsynchronouslyWithoutDeadlocking:(void (^)())block
{
    ...
    {
        [DNBUtilities runOnMainThreadAfterDelay:0.01f
                                          block:
         ^()
         {
             block();
...
{
    [DNBUtilities runOnMainThreadWithoutDeadlocking:^
     {
         block();
     }];
}
...
+ (NSTimer*)repeatRunAfterDelay:(CGFloat)delay block:(void (^)())block
{
    ...
    return [NSTimer scheduledTimerWithTimeInterval:delay target:[DNBUtilities sharedInstance] selector:@selector(instanceRunBlock:) userInfo:block_ repeats:YES];
}
...
+ (NSTimer*)runTimerAfterDelay:(CGFloat)delay block:(void (^)())block
{
    ...
    return [NSTimer scheduledTimerWithTimeInterval:delay target:[DNBUtilities sharedInstance] selector:@selector(instanceRunBlock:) userInfo:block_ repeats:NO];
}
...
+ (void)runGroupWithTimeout:(dispatch_time_t)timeout
         onBackgroundThread:(void (^)(dispatch_group_t group))block
             withCompletion:(void (^)())completionBlock
{
    [DNBUtilities runOnBackgroundThread:
     ^()
     {
...
+(AVAudioPlayer*)createSound:(NSString*)fName ofType:(NSString*)ext
{
    ...
    {
        DNBLog(DNBLL_Debug, DNBLD_General, @"error, file not found: %@", path);
    }
...
+ (void)playSound:(NSString*)name
{
    ...
    dispatch_once(&once, ^{
        avSound_buzz    = [DNBUtilities createSound:@"buzz_Error" ofType:@"mp3"];
        avSound_ding    = [DNBUtilities createSound:@"ding_Capture" ofType:@"mp3"];
        avSound_tada    = [DNBUtilities createSound:@"tada_Reward" ofType:@"mp3"];
        avSound_beep    = [DNBUtilities createSound:@"beep_Scanner" ofType:@"mp3"];
    });
    ...
    [DNBUtilities runAfterDelay:3.0f
                          block:
     ^()
     {
         //NSError*    error;
...
+ (NSString*)dictionaryString:(NSDictionary*)dictionary dirty:(BOOL*)dirtyFlag withItem:(NSString*)key andDefault:(NSString*)defaultValue
{
    ...
    return [retval dnbStringByDecodingXMLEntities];
}
...
{
    return [[self class] dictionaryDNBDate:dictionary dirty:nil withItem:key andDefault:defaultValue];
}
    ...
    if (object != nil)
    {
        if ([object isKindOfClass:[NSNumber class]])
        {
            if (object != (NSNumber*)[NSNull null])
            {
                ...
                NSDate*     localTime   = [gmtTime dnbToGlobalTime];
                
                DNBDate*    newval = [DNBDate dateWithComponentFlags:dateFlags fromDate:localTime];
                ...
                if (newval == nil)
                {
                    ...
                    if (timestamp != nil)
                    {
                        if ([timestamp integerValue] != 0)
                        {
                            ...
                            newval = [DNBDate dateWithComponentFlags:dateFlags fromDate:localTime];
                        }
                    }
                }
                if (newval)
        ...
        else if ([object isKindOfClass:[DNBDate class]])
        {
            if (object != (DNBDate*)[NSNull null])
            {
                DNBDate*   newval  = object;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        logDebugLevel   = DNBLL_Everything;
...
{
    return [DNBUtilities sharedInstance].xcodeColorsEscape;
}

+ (NSString*)xcodeColorsReset
{
    return [DNBUtilities sharedInstance].xcodeColorsReset;
}

+ (NSString*)xcodeColorsLevelColor:(DNBLogLevel)level
{
    if ([DNBUtilities sharedInstance].xcodeColorsEscape.length == 0)
    {
        return @"";
    }
    ...
    switch (level)
    {
        case DNBLL_Critical:       {   color   = XCODE_COLORS_ESCAPE @"fg255,255,255;" XCODE_COLORS_ESCAPE @"bg255,0,0;";  break;  }
        case DNBLL_Error:          {   color   = XCODE_COLORS_ESCAPE @"fg255,0,0;" XCODE_COLORS_ESCAPE @"bg;";             break;  }
        case DNBLL_Warning:        {   color   = XCODE_COLORS_ESCAPE @"fg255,127,0;" XCODE_COLORS_ESCAPE @"bg;";           break;  }
        case DNBLL_Debug:          {   color   = XCODE_COLORS_ESCAPE @"fg0,0,255;" XCODE_COLORS_ESCAPE @"bg;";             break;  }
        case DNBLL_Info:           {   color   = XCODE_COLORS_ESCAPE @"fg100,100,133;" XCODE_COLORS_ESCAPE @"bg;";         break;  }
        case DNBLL_Everything:     {   color   = XCODE_COLORS_ESCAPE @"fg64,64,64;" XCODE_COLORS_ESCAPE @"bg;";            break;  }
    }
...
+ (NSString*)xcodeColorsDomainColor:(NSString*)domain
{
    ...
    if ([domain isEqualToString:DNBLD_UnitTests])          {   color   = XCODE_COLORS_ESCAPE @"fg255,255,255;" XCODE_COLORS_ESCAPE @"bg255,0,0;"; }
    else if ([domain isEqualToString:DNBLD_General])       {   color   = XCODE_COLORS_ESCAPE @"fg80,80,80;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_Framework])     {   color   = XCODE_COLORS_ESCAPE @"fg255,0,0;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_CoreData])      {   color   = XCODE_COLORS_ESCAPE @"fg255,255,255;" XCODE_COLORS_ESCAPE @"bg0,0,255;"; }
    else if ([domain isEqualToString:DNBLD_CoreDataIS])    {   color   = XCODE_COLORS_ESCAPE @"fg255,252,229;" XCODE_COLORS_ESCAPE @"bg0,0,255;"; }
    else if ([domain isEqualToString:DNBLD_Realm])         {   color   = XCODE_COLORS_ESCAPE @"fg0,127,0;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_ViewState])     {   color   = XCODE_COLORS_ESCAPE @"fg255,252,229;" XCODE_COLORS_ESCAPE@"bg0,127,0;"; }
    else if ([domain isEqualToString:DNBLD_Theming])       {   color   = XCODE_COLORS_ESCAPE @"fg255,255,255;" XCODE_COLORS_ESCAPE @"bg0,127,0;"; }
    else if ([domain isEqualToString:DNBLD_Location])      {   color   = XCODE_COLORS_ESCAPE @"fg255,127,0;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_Networking])    {   color   = XCODE_COLORS_ESCAPE @"fg0,0,255;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_API])           {   color   = XCODE_COLORS_ESCAPE @"fg0,255,127;" XCODE_COLORS_ESCAPE @"bg;"; }
    else if ([domain isEqualToString:DNBLD_DAO])           {   color   = XCODE_COLORS_ESCAPE @"fg0,255,127;" XCODE_COLORS_ESCAPE @"bg;"; }
...
{
    [self logEnableDomain:domain forLevel:DNBLL_Everything];
}
...
{
    [self logDisableDomain:domain forLevel:DNBLL_Everything];
}
...
levelString(int level)
{
    switch (level)
    {
        case DNBLL_Critical:    {   return @"🆘🆘🆘";   }
        case DNBLL_Error:       {   return @"🚫🚫🚫";   }
        case DNBLL_Warning:     {   return @"⚠️⚠️⚠️";   }
        case DNBLL_Debug:       {   return @"🐞🐞🐞";   }
        case DNBLL_Info:        {   return @"✳️✳️✳️";   }
        case DNBLL_Everything:  {   return @"👀👀👀";   }
    }
...
{
    if ([[DNBUtilities sharedInstance] isLogEnabledDomain:domain andLevel:level] != YES)
    {
        return;
    }
    
    NSString*   domainColor = [DNBUtilities xcodeColorsDomainColor:domain];
    NSString*   mainColor   = [DNBUtilities xcodeColorsLevelColor:level];
    NSString*   otherColor  = [DNBUtilities xcodeColorsOtherColor];
    ...
    NSLog(@"%@ %@[%@] %@{%@} %@[%@:%d] %@%@%@", levelString(level), otherColor, ([NSThread isMainThread] ? @"MT" : @"BT"), domainColor, domain, otherColor, [NSString stringWithUTF8String:filename].lastPathComponent, lineNumber, mainColor, formattedStr, DNBUtilities.xcodeColorsReset);
}
